...
const AppContent = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      ...
      if (event.key === "Delete") {
          ...
          case 'c':
            if (event.ctrlKey || event.metaKey) {
              ...
              if (selectedRange.length === 2) {
                ...
                // Если выбрана только одна ячейка, создаем диапазон из нее
                copyRange([[selectedCell.row, selectedCell.col], [selectedCell.row, selectedCell.col]], rows);
              }
              return;
            }
            break;
        ...
        setSelectedRange([]);
        // Обновляем формулу для новой выбранной ячейки
  ...
  const insertSingleCellIntoFormula = () => {
    ...
    const handleSaveAs = () => {
      const format = prompt("Введите формат для файла (xlsx, xlsm):").toLowerCase();
